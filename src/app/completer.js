...
export default class Completer {
   ...
   */
  getCompleterWordList(list, prefix, modules) {
    let results = [];

    for (let item of list) {
      if (item.word.toLowerCase().includes(prefix.toLowerCase().split(''))) {
        if (modules.includes(item.meta.toLowerCase().split(' ')[0])) {
          results.push(item);
        }
      }
    }
